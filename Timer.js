...
function cahangeText(){
    timeoutput = `${h} : ${m} :  ${s}`; // Найди переменные из тела и перенеси их в поле вывода
}

function checkTimer() {
    if (h == 0 && m == 0&&s==0){
        return true;
    }else{return false} //Можно ли убрать полность else, по сути же если не истина, то не входи, а значит ничего не делай
}